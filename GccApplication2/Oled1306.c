...
{
	0xAE,0xA8,0x3f,0xD3,0x00,0x40,0xA1,0xC8,0xDA,0x12,0x81,0x7F,0xA4,0xA6,0xD5,0x80,0x8D,0x14,0x20,0x00,0xA1,0xC8,0xAF  
};
//
...
void ini_oled(void)
{
	...
	{
		TWI_SendByte(NEXT_COMMAND);
		TWI_SendByte(pgm_read_byte(&init[i]));
		//usart0_transmit_data((uint8_t*)&TWSR,1);
	}
	TWI_StopCondition();
	clear_oled();
}
...
void clear_oled(void)
{
	...
	for (uint8_t page=0;page<PAGE_128_64;page++)
	{
		for(uint8_t col=0;col<COLUM_128_64;col++)
		{
			TWI_SendByte(0x00);
		}
	}
...
void oled_write(uint8_t mode, uint8_t data) // Режим: 1-данные, 0-команда
{
	uint8_t m;
	if(mode) m = NEXT_DATA; // Режим данных
	else m = NEXT_COMMAND; // Режим команды

	TWI_StartCondition();			//send start strings
	TWI_Send_address(ADDRESS);		//Slave address
	TWI_SendByte(m);		//Control Byte - Command
	TWI_SendByte(data);		//payload
	TWI_StopCondition();
}
